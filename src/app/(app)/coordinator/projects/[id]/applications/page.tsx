...
export default async function ProjectApplicationsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div><Link href="/coordinator/projects" className="inline-flex items-center gap-2 text-sm font-bold text-green-700 hover:underline"><ArrowLeft className="size-4" />К проектам</Link><div className="mt-5"><p className="text-sm font-bold text-green-700">Управление заявками</p><h1 className="mt-1 text-3xl font-black tracking-tight">{project.title}</h1><p className="mt-2 text-sm text-muted-foreground">Принимайте участников, отмечайте посещение и подтверждайте фактические часы.</p></div><div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><StatCard label="Всего заявок" value={managed.length} icon={UsersRound} /><StatCard label="Новые" value={managed.filter((item) => item.status === "pending").length} icon={UserCheck} tone="amber" /><StatCard label="Одобрено" value={managed.filter((item) => ["approved", "attended", "completed"].includes(item.status)).length} icon={CheckCircle2} tone="blue" /><StatCard label="Часы подтверждены" value={(hours ?? []).filter((item) => item.status === "confirmed").reduce((sum, item) => sum + Number(item.hours), 0)} icon={Clock3} tone="purple" /></div><div className="mt-7"><ApplicationManager applications={managed} projectHours={project.volunteer_hours} /></div></div>;
}
