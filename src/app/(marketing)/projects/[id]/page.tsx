...
export default async function ProjectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <section className="py-10 sm:py-14"><div className="page-shell"><Link href="/projects" className="text-sm font-bold text-primary hover:underline">← Назад к проектам</Link><div className="mt-7 grid gap-8 lg:grid-cols-[minmax(0,1fr)_23rem]"><div><ProjectCover src={project.cover_url} title={project.title} priority className="aspect-[16/8] rounded-[1.75rem]" /><div className="mt-7 flex flex-wrap gap-2"><Badge variant="secondary">{project.category}</Badge><StatusBadge status={project.status} /><Badge variant="outline">{project.format === "online" ? "Онлайн" : "Офлайн"}</Badge></div><h1 className="mt-5 text-balance text-4xl font-black tracking-tight sm:text-5xl">{project.title}</h1><p className="mt-6 whitespace-pre-line text-lg leading-8 text-muted-foreground">{project.description}</p><div className="mt-10 grid gap-4 sm:grid-cols-2"><Info icon={CalendarDays} label="Начало" value={formatDate(project.start_date, "d MMMM yyyy, HH:mm")} /><Info icon={CalendarDays} label="Окончание" value={formatDate(project.end_date, "d MMMM yyyy, HH:mm")} /><Info icon={project.format === "online" ? Laptop : MapPin} label={project.format === "online" ? "Формат" : "Место"} value={project.format === "online" ? "Онлайн" : `${project.city}, ${project.address}`} /><Info icon={Clock3} label="Волонтёрские часы" value={`${project.volunteer_hours} часов`} /></div>{project.requirements ? <div className="mt-10 rounded-2xl border bg-surface p-6"><h2 className="text-xl font-black">Требования к участникам</h2><div className="mt-4 flex items-start gap-3 text-sm leading-7 text-muted-foreground"><CheckCircle2 className="mt-1 size-5 shrink-0 text-success" /><p className="whitespace-pre-line">{project.requirements}</p></div></div> : null}</div><aside className="space-y-5 lg:sticky lg:top-24 lg:self-start"><Card><CardContent className="p-6"><div className="grid grid-cols-2 gap-4"><Metric icon={Users} value={project.participantCount ?? 0} label="участников" /><Metric icon={ShieldCheck} value={project.availablePlaces ?? project.required_volunteers} label="свободных мест" /></div><div className="my-6 border-t" /><ApplicationButton projectId={project.id} userRole={profile?.role} application={application} disabled={unavailable} /><p className="mt-4 text-center text-xs leading-5 text-muted-foreground">Координатор рассмотрит заявку и обновит её статус в вашем кабинете.</p></CardContent></Card><Card><CardContent className="p-6"><p className="text-xs font-bold uppercase tracking-widest text-muted-foreground">Координатор</p><div className="mt-4 flex items-center gap-3"><Avatar className="size-12"><AvatarImage src={project.coordinator?.avatar_url ?? undefined} /><AvatarFallback>{initials(project.coordinator?.full_name)}</AvatarFallback></Avatar><div><p className="font-black">{project.coordinator?.full_name ?? "Координатор Qamqor"}</p><p className="mt-1 text-xs text-muted-foreground">{project.coordinator?.city ?? project.city}</p></div></div></CardContent></Card><a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${project.city} ${project.address ?? ""}`)}`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 text-sm font-bold text-primary hover:underline">Открыть на карте <ExternalLink className="size-4" /></a></aside></div></div></section>;
}
